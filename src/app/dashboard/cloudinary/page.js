...
function CredentialField({ label, value, onChange, secret = false }) {
  return <label className="block"><span className="mb-1 block text-xs font-semibold text-slate-600 dark:text-slate-300">{label}</span><input required type={secret ? 'password' : 'text'} value={value} onChange={(event) => onChange(event.target.value)} autoComplete="off" className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 dark:border-slate-600 dark:bg-slate-800 dark:text-white" /></label>;
}
...
export default function CloudinaryPage() {
  ...
  const [migrationForm, setMigrationForm] = useState({
    source: { cloud_name: '', api_key: '', api_secret: '' },
    destination: { cloud_name: '', api_key: '', api_secret: '' },
    overwrite: true,
  });
  ...
  const deliveredCount = useMemo(() => assets.filter((asset) => asset.status === 'Delivered').length, [assets]);
  const storageBytes = useMemo(() => assets.reduce((total, asset) => total + (Number(asset.bytes) || 0), 0), [assets]);
  const stats = useMemo(() => ({
    total: loading ? '—' : assets.length,
    storage: loading ? '—' : formatBytes(storageBytes),
    delivered: loading ? '—' : deliveredCount,
    selected: selected.length,
  }), [assets.length, deliveredCount, loading, selected.length, storageBytes]);
  ...
  const closeMigration = () => {
    ...
    setMigrationForm({ source: { cloud_name: '', api_key: '', api_secret: '' }, destination: { cloud_name: '', api_key: '', api_secret: '' }, overwrite: true });
  };
  const updateAccount = (account, field, value) => setMigrationForm((current) => ({ ...current, [account]: { ...current[account], [field]: value } }));
  const startMigration = async (event) => {
    event.preventDefault();
    setMigrating(true);
    setMigrationResult(null);
    try {
      const { data: started } = await api.post('/cloudinary/migrate', migrationForm);
      const poll = async () => {
        const { data } = await api.get(`/cloudinary/migrate/${started.jobId}`);
        setMigrationProgress(data);
        if (data.status === 'completed') {
          setMigrationResult(data);
          notify(`Migration finished: ${data.migrated} migrated, ${data.database?.referencesUpdated || 0} database references updated`);
          return;
        }
      ...
      setMigrationResult({ error: error.response?.data?.message || 'Migration failed. Check the credentials and try again.' });
    } finally {
      setMigrating(false);
      setMigrationForm((current) => ({ ...current, source: { cloud_name: '', api_key: '', api_secret: '' }, destination: { cloud_name: '', api_key: '', api_secret: '' } }));
    }
  };
  ...
  return <ProtectedRoute><div className="flex min-h-screen bg-slate-50 dark:bg-slate-950">
    ...
    <div className="flex min-w-0 flex-1 flex-col">
      ...
      <main className="flex-1 px-4 py-8 sm:px-6 lg:px-8"><div className="mx-auto max-w-7xl">
        <div className="mb-8 flex flex-col justify-between gap-5 md:flex-row md:items-end"><div><div className="mb-3 flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-indigo-600"><span className="h-2 w-2 rounded-full bg-emerald-500" />Connected to Cloudinary</div><h1 className="text-3xl font-bold tracking-tight text-slate-900 dark:text-white">Media library</h1><p className="mt-2 text-sm text-slate-500 dark:text-slate-400">Organize, inspect, and publish the visual assets powering your platform.</p></div><div className="flex flex-wrap gap-3"><button type="button" onClick={() => setUploadOpen(true)} className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-5 py-3 text-sm font-semibold text-slate-700 shadow-sm hover:border-indigo-300 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200"><Icon d="M12 4v16m8-8H4" className="h-4 w-4" />Upload assets</button><button type="button" onClick={() => setMigrationOpen(true)} className="inline-flex items-center justify-center gap-2 rounded-xl bg-indigo-600 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-indigo-600/20 hover:bg-indigo-700"><Icon d="M4 7h16M4 12h16M4 17h16" className="h-4 w-4" />Migrate account</button></div></div>
        <div className="mb-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Stat label="Total assets" value={stats.total} hint={`${assets.filter((asset) => asset.type === 'image').length} images · ${assets.filter((asset) => asset.type === 'video').length} videos`} color="bg-indigo-600" d="M4 6a2 2 0 012-2h2l2-2h4l2 2h2a2 2 0 012 2v10a2 2 0 01-2 2H6a2 2 0 01-2-2V6zm8 10a4 4 0 100-8 4 4 0 000 8z" /><Stat label="Storage used" value={stats.storage} hint={`${assets.length ? `${assets.filter((asset) => asset.type === 'raw').length} documents` : 'No assets loaded'}`} color="bg-violet-600" d="M4 7a2 2 0 012-2h12a2 2 0 012 2v10a2 2 0 01-2 2H6a2 2 0 01-2-2V7zm0 3h16m-4 4h.01" /><Stat label="Delivered" value={stats.delivered} hint="Ready for production" color="bg-emerald-600" d="M5 13l4 4L19 7" /><Stat label="Selected" value={stats.selected} hint={selected.length ? 'Ready for bulk actions' : 'Select assets to manage'} color="bg-amber-500" d="M9 11l3 3L22 4M4 4h5v5H4zM4 15h5v5H4z" /></div>
    ...
    {migrationOpen && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4 backdrop-blur-sm" onClick={closeMigration}><form onSubmit={startMigration} className="w-full max-w-2xl rounded-2xl bg-white p-6 shadow-2xl dark:bg-slate-900" onClick={(event) => event.stopPropagation()}><div className="flex items-start justify-between"><div><h2 className="text-xl font-bold text-slate-900 dark:text-white">Migrate Cloudinary assets</h2><p className="mt-1 text-sm text-slate-500">Credentials are used only for this request and are never stored.</p></div><button type="button" disabled={migrating} onClick={closeMigration} className="text-slate-400"><Icon d="M6 18L18 6M6 6l12 12" /></button></div><div className="mt-6 grid gap-6 md:grid-cols-2">{[['source', 'Previous account'], ['destination', 'New account']].map(([account, title]) => <div key={account} className="space-y-3 rounded-xl border border-slate-200 p-4 dark:border-slate-700"><h3 className="font-semibold text-slate-800 dark:text-slate-200">{title}</h3><CredentialField label="Cloud name" value={migrationForm[account].cloud_name} onChange={(value) => updateAccount(account, 'cloud_name', value)} /><CredentialField label="API key" value={migrationForm[account].api_key} onChange={(value) => updateAccount(account, 'api_key', value)} /><CredentialField label="API secret" secret value={migrationForm[account].api_secret} onChange={(value) => updateAccount(account, 'api_secret', value)} /></div>)}</div><label className="mt-4 flex items-center gap-2 text-sm text-slate-600 dark:text-slate-300"><input type="checkbox" checked={migrationForm.overwrite} onChange={(event) => setMigrationForm((current) => ({ ...current, overwrite: event.target.checked }))} className="h-4 w-4 rounded border-slate-300 text-indigo-600" />Overwrite matching assets in the new account</label>{migrationProgress && <div className="mt-5 rounded-xl bg-indigo-50 p-4 dark:bg-indigo-950/30"><div className="flex justify-between text-sm font-semibold text-indigo-900 dark:text-indigo-200"><span>{migrationProgress.status === 'completed' ? 'Migration complete' : 'Migrating assets...'}</span><span>{migrationProgress.processed || 0} / {migrationProgress.total || '...'}</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-indigo-100 dark:bg-indigo-900"><div className="h-full rounded-full bg-indigo-600 transition-all" style={{ width: `${migrationProgress.total ? Math.round((migrationProgress.processed / migrationProgress.total) * 100) : 5}%` }} /></div><p className="mt-2 text-xs text-indigo-700 dark:text-indigo-300">Migrated: {migrationProgress.migrated || 0} · Skipped: {migrationProgress.skipped || 0} · Failed: {migrationProgress.failed || 0}</p></div>}{migrationResult && <div className={`mt-4 rounded-lg p-3 text-sm ${migrationResult.error ? 'bg-rose-50 text-rose-700' : 'bg-emerald-50 text-emerald-700'}`}>{migrationResult.error || `Finished: ${migrationResult.migrated} migrated, ${migrationResult.skipped} skipped, ${migrationResult.failed} failed out of ${migrationResult.total}. Database references updated: ${migrationResult.database?.referencesUpdated || 0} across ${migrationResult.database?.documentsUpdated || 0} documents.`}</div>}<div className="mt-6 flex justify-end gap-3"><button type="button" disabled={migrating} onClick={closeMigration} className="rounded-lg px-4 py-2 text-sm font-semibold text-slate-500 hover:bg-slate-100">Close</button><button type="submit" disabled={migrating} className="rounded-lg bg-indigo-600 px-5 py-2 text-sm font-semibold text-white hover:bg-indigo-700 disabled:cursor-not-allowed disabled:opacity-60">{migrating ? 'Migrating...' : 'Start migration'}</button></div></form></div>}
