...
export default function ReportedPostsPage() {
  ...
  const navigation = [
    ...
    { name: 'Reported Posts', href: '/dashboard/reports/posts', icon: 'M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z', current: true },
    { name: 'Reported Profiles', href: '/dashboard/reports/profiles', icon: 'M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z', current: false },
  ...
  useEffect(() => {
    const fetchReports = async () => {
      try {
        setLoading(true);
        const res = await api.get('/reports/posts');
        setReports(res.data || []);
      } catch (e) {
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <main className="flex-1 py-8">
            <div className="px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
              <div className="mb-8">
                <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Reported Posts</h1>
                <p className="mt-2 text-gray-600 dark:text-gray-400">Review and manage reported posts.</p>
              </div>
                  ...
                  <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                      {loading ? (
                        ...
                        reports.map((r) => {
                          const authorName = r.targetPost?.author?.fullName || r.targetPost?.author?.displayName || 'Unknown';
                            ...
                            <tr key={r._id}>
                              ...
                              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                                <div className="flex items-center space-x-2">
                                  <span className="text-gray-700 dark:text-gray-300">{authorName}</span>
                                  {r.targetPost?._id && (
                                    <Link href={`/dashboard/posts/${r.targetPost._id}`} className="text-indigo-600 dark:text-indigo-400 hover:underline">View</Link>
                                  )}
                                </div>
                              </td>
