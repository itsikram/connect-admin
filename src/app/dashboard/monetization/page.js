...
export default function MonetizationPage() {
  ...
  const updatePayout = (field, value) => setSettings((current) => ({ ...current, payout: { ...current.payout, [field]: value } }));
          ...
          <div className="mx-auto max-w-5xl space-y-6 px-4 py-8 sm:px-6 lg:px-8">
            ...
            {loading ? <div className="rounded-xl bg-white p-8 text-center text-gray-500 dark:bg-gray-800">Loading settings…</div> : (
              <>
                ...
                <section className="rounded-xl bg-white p-6 shadow-sm dark:bg-gray-800"><h2 className="text-lg font-bold text-gray-900 dark:text-white">Earnings and payouts</h2><p className="mt-1 text-sm text-gray-500">These values apply to new action rewards and payout requests.</p><div className="mt-4 grid gap-4 sm:grid-cols-3"><label className="text-xs text-gray-500">BDT per coin<input type="number" min="0.000001" step="0.01" value={settings.payout.coinToBDTRate} onChange={(event) => updatePayout('coinToBDTRate', Number(event.target.value))} className="mt-1 w-full rounded border px-2 py-2 text-sm dark:border-gray-600 dark:bg-gray-700 dark:text-white" /></label><label className="text-xs text-gray-500">Minimum payout coins<input type="number" min="1" value={settings.payout.minimumCoins} onChange={(event) => updatePayout('minimumCoins', Number(event.target.value))} className="mt-1 w-full rounded border px-2 py-2 text-sm dark:border-gray-600 dark:bg-gray-700 dark:text-white" /></label><label className="text-xs text-gray-500">Coins per completed action<input type="number" min="1" value={settings.payout.actionRewardCoins} onChange={(event) => updatePayout('actionRewardCoins', Number(event.target.value))} className="mt-1 w-full rounded border px-2 py-2 text-sm dark:border-gray-600 dark:bg-gray-700 dark:text-white" /></label></div></section>
